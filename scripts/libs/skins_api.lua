...
AddClassPostConstruct("screens/redux/lobbyscreen", function(self, profile, cb)
	local function OnUserPickedCharacter(char, skin_base, clothing_body, clothing_hand, clothing_legs, clothing_feet)
		local function doSpawn()
			...
			SendModRPCToServer(GetModRPC("modded_skins", "moddedskins"), skin_base)
		end
...
AddClassPostConstruct("screens/redux/wardrobepopupgridloadout", function(self)
	...
	function self:Close(...)
		if not self.owner_player or not SKINNABLE_CHARACTERS[self.owner_player.prefab] then
			return _Close(self, ...)
		end
...
local _IsDefaultSkinOwned = IsDefaultSkinOwned
local _ValidateItemsLocal = ValidateItemsLocal
local _SpawnNewPlayerOnServerFromSim = SpawnNewPlayerOnServerFromSim
local _GetLockedSkinFilter = GetLockedSkinFilter
...
function SpawnNewPlayerOnServerFromSim(player_guid, ...)
	local player = Ents[player_guid]
	if player and SKINNABLE_CHARACTERS[player.prefab] then
		local skinner = player.components.skinner
		player:DoTaskInTime(0, function()
			skinner:SetSkinName(skinner.skin_name)
		end)
	end
	return _SpawnNewPlayerOnServerFromSim(player_guid, ...)
end

if not MOD_RPC["modded_skins"] then
	AddModRPCHandler("modded_skins", "moddedskins", function(inst, base)
