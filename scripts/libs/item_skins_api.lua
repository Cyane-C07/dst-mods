    ...
    self.GetSkinOptions = function(self, ...)
        ...
        if self.skins_list and TheNet:IsOnlineMode() then 
            for i, data in ipairs(self.skins_list) do
				...
				local atlas = image_data
					and self.recipe.skin_img_data[item].atlas or self.recipe.atlas
...
local _CheckOwnership = InventoryProxy.CheckOwnership
function InventoryProxy:CheckOwnership(item, ...)
	if IsSkinModded(item) then
		return true
	end
	return _CheckOwnership(self, item, ...)
end

local _GetFullInventory = InventoryProxy.GetFullInventory
function InventoryProxy:GetFullInventory(...)
	local inv = _GetFullInventory(self, ...)

	for skin, _ in pairs(MODDED_SKINS) do
		table.insert(inv, {
			item_id = "0",
			item_type = skin,
			modified_time = 0,
