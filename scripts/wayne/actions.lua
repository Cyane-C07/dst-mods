...
REFILL.id = "WAYNE_REFILL"

REFILL.fn = function(act)
    if act.invobject and act.doer and act.target then
        return act.invobject.components.wayne_bottle:Refill(act.doer, act.target)
    end
end
...
AddComponentAction("EQUIPPED", "wayne_bottle", function(inst, doer, target, actions, right)
...
AddComponentAction("USEITEM", "wayne_bottle", function(inst, doer, target, actions, right)
	if doer:HasTag("shadowartist") and target:HasTag("shadow_ink_source") and
	GetFiniteusesPercent(inst) ~= 1 then
		table.insert(actions, REFILL)
	end
...
DRAW.fn = function(act)
    if act.invobject and act.doer and act.target then
        return act.invobject.components.wayne_paper:Draw(act.target, act.doer)
    end
end

AddAction(DRAW)

AddComponentAction("USEITEM", "wayne_paper", function(inst, doer, target, actions, right)
	if not doer:HasTag("shadowartist") or target:HasTag("wayne_drawnitem") then
		return
	end
...
AddStategraphActionHandler("wilson", ActionHandler(DRAW, "wayne_draw"))
AddStategraphActionHandler("wilson_client", ActionHandler(DRAW, "wayne_draw"))

----------------------------------------------------------------------------------------------

local FEATHER = Action({priority=1})
FEATHER.str = STRINGS.ACTIONS.USE_FEATHER
FEATHER.id = "WAYNE_FEATHER"
FEATHER.distance = math.huge

FEATHER.fn = function(act)
    if act.doer:HasTag("shadowartist") and act.invobject then
        return act.invobject.components.wayne_feather:Use(act.doer)
    end
end

AddAction(FEATHER)

AddComponentAction("POINT", "wayne_feather", function(inst, doer, pos, actions, right)
	if doer:HasTag("shadowartist") and right then
		table.insert(actions, FEATHER)
	end
end)

AddStategraphActionHandler("wilson", ActionHandler(FEATHER, "wayne_feather_pre"))
AddStategraphActionHandler("wilson_client", ActionHandler(FEATHER, "wayne_feather_pre"))
...
--     if act.invobject and act.invobject.components.wayne_bottle then
--         return act.invobject.components.wayne_bottle:PourOut(act:GetActionPoint(), act.doer)
-- 	end
-- end
...
-- env.AddComponentAction("POINT", "wayne_bottle", function(inst, doer, pos, actions, right)
-- 	local x, y, z = pos:Get()
-- 	if doer:HasTag("wayne") and not right and GetPercent(inst) == 1 and
...
-- SWALLOW.str = "Swallow"
-- SWALLOW.id = "WAYNE_SWALLOW"

-- SWALLOW.fn = function(act)
--     if act.invobject and act.invobject.components.wayne_bottle then
--         return act.invobject.components.wayne_bottle:Swallow(act.doer)
-- 	end
-- end

-- env.AddAction(SWALLOW)

-- env.AddComponentAction("INVENTORY", "wayne_bottle", function(inst, doer, actions, right)
-- 	if doer:HasTag("wayne") and GetPercent(inst) > 0 then
