...
local function MakeFx(t)
	local assets = {}
	if not t.noassets then
		table.insert(assets, Asset(t.dynamic and "DYNAMIC_ANIM" or "ANIM", "anim/"..(t.dynamic and "dynamic/"..t.build..".dyn" or t.build..".zip")))
	end
	...
	local function startfx(proxy)
		...
		if t.sound ~= nil then
			...
			local s = type(t.sound)
			if s == "table" then
				for i, data in pairs(t.sound) do
					inst:DoTaskInTime(data[1], PlaySound, data[2])
				end
